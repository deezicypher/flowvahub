...
export const signup = async (req: Request, res: Response) => {
    ...
    try {
      ...
      const query = "SELECT * FROM users WHERE email = $1 O";
   ...
   if (rows.length > 0) {
    ...
    if (user.email === normalizedEmail) {
      ...
      const result = await pool.query(saveQuery, [normalizedName, normalizedEmail, hashedPass]);
  
      const { id, created_at} = result.rows[0]
      // Respond with a success message and the created user
     res.json({ msg: 'Account created successfully! Welcome to Flowva.', user: {id,name,email,created_at} });
